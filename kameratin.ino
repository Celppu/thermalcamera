...
#ifdef ESP8266
   #define STMPE_CS -1 // 16
   #define TFT_CS   15 // select
   #define TFT_DC   2 // 15
   #define SD_CS    -1 //sdkortti?
...
void loop() {
 ...
 //  tft.fillRect(40, 40, 20, 20, camColors[142]);
  ...
  uint16_t boxsize = min(160 / INTERPOLATED_COLS, 128 / INTERPOLATED_COLS);
  ...
  for(int i=1; i<=AMG88xx_PIXEL_ARRAY_SIZE; i++){
    ...
    if(pixels[i-1] > maxim) maxim = pixels[i-1];
...
void drawpixels(float *p, uint8_t rows, uint8_t cols, uint8_t boxWidth, uint8_t boxHeight, boolean showVal) {
  ...
  for (int y=0; y<rows; y++) {
    for (int x=0; x<cols; x++) {
      ...
      uint16_t color;
      color = val * 2;
      tft.fillRect( boxWidth*y, boxWidth*x, boxWidth, boxHeight, camColors[colorIndex]);
      ...
      if (showVal) {
       ...
       // tft.setTextColor(TFT_WHITE);
        tft.setCursor(120, 100);
        tft.print(20.5,1);
        //tft.drawCentreString(String(30), 80, 14, 2);
      }
    } 
  }
